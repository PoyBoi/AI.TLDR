VLLM:
- https://docs.vllm.ai/en/latest/usage/
- https://github.com/vllm-project/vllm/tree/main/vllm/model_executor/models

MLOPS:
- https://machinelearningmastery.com/10-must-know-python-libraries-for-mlops-in-2025/

DOCKER:
- https://www.youtube.com/watch?v=FvdlwyMwrzg
- https://www.youtube.com/watch?v=DQdB7wFEygo (_helpful_)
- https://www.youtube.com/watch?v=GOgfQxDPaDw

LANGCHAIN + VLLM:
- https://docs.langchain.com/oss/python/integrations/chat/vllm
- https://docs.vllm.ai/en/stable/serving/integrations/langchain/
- https://reference.langchain.com/python/langchain-community/llms/vllm/VLLM
    - API:
        - https://reference.langchain.com/python/langchain-openai/chat_models/base/ChatOpenAI

CODING AND ADV PYTHON
- https://www.w3schools.com/Python/python_args_kwargs.asp